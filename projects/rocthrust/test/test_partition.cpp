...
__global__ THRUST_HIP_LAUNCH_BOUNDS_DEFAULT void PartitionKernel(int const N, int* array)
{
  if (threadIdx.x == 0)
  {
...
PartitionCopyKernel(int const N, int* array, int* true_arr, int* false_arr, int* size_array)
{
  if (threadIdx.x == 0)
  {
    thrust::device_ptr<int> begin(array);
    ...
    thrust::pair<iter, iter> end_pair =
      thrust::partition_copy(thrust::hip::par, begin, end, true_begin, false_begin, is_even<int>());
...
TEST(PartitionTests, TestPartitionCopyDevice)
{
  ...
  for (auto size : get_sizes())
  {
    ...
    for (auto seed : get_seeds())
    {
      ...
      auto end_pair =
        thrust::partition_copy(h_data.begin(), h_data.end(), h_true.begin(), h_false.begin(), is_even<int>());
