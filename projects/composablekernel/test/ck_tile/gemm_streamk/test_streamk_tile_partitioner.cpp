...
TEST(StreamKTilePartitionerBaseRemapXCD, SmallArray)
{
    ...
    if constexpr(CompilerTargetT::value == core::arch::amdgcn_target_id::GFX942)
    {
        ...
        ck_tile::StreamKTilePartitioner<Config::GemmShape,
        ...
        test_remap_xcd<Config::GemmShape>(
            initial_values, expected_values, tile_partitioner, num_xcds);
    }
    else
    {
        GTEST_SKIP() << "Skipping: not gfx942 build";
    }
}
...
TEST(StreamKTilePartitionerBaseRemapXCD, MidArray)
{
    ...
    if constexpr(CompilerTargetT::value == core::arch::amdgcn_target_id::GFX942)
    {
        ...
        const std::vector<ck_tile::index_t> expected_values = {
            ...
            14, 30, 46, 62, 78, 94, 110, 126, 15, 31, 47, 63, 79, 95, 111};
        test_remap_xcd<Config::GemmShape>(
            initial_values, expected_values, tile_partitioner, num_xcds);
    }
    else
...
TEST(StreamKTilePartitionerBaseRemapXCD, UnevenXCD)
{
    ...
    if constexpr(CompilerTargetT::value == core::arch::amdgcn_target_id::GFX942)
    {
        ...
        test_remap_xcd<Config::GemmShape>(initial_values, expected_values, tile_partitioner, 5);
    }
    else
    {
        GTEST_SKIP() << "Skipping: not gfx942 build";
